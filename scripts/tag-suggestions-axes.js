...
- energy_level: ${VALID.energy_level.join(' / ')}（その提案の実行に必要なエネルギーレベル。複数可、不問なら空配列）
- social_context: ${VALID.social_context.join(' / ')}（一人 alone / 他者と with_others のどちらに適するか、両方OKなら空配列）
- time_pressure: ${VALID.time_pressure.join(' / ')}（relaxed=余裕がある / pressed=急いでいる、両方OKなら空配列）
...
async function fetchTargets(supabase, { limit, onlyUntagged }) {
  let query = supabase
    ...
    .select('id, title, description, duration, category, steps, season, weather, temperature_band, part_of_day, day_type, mood, intent, seasonal_events, energy_level, social_context, time_pressure, is_universal')
    .eq('is_public', true)
