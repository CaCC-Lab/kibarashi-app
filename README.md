# 🧘 気晴らしレシピ - AIによる音声ガイド付きストレス解消アプリ

<div align="center">

[![Live Demo](https://img.shields.io/badge/🚀%20デモサイト-今すぐ体験-brightgreen?style=for-the-badge)](https://kibarashi-app.vercel.app/)
[![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)](https://www.typescriptlang.org/)
[![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)](https://reactjs.org/)
[![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)](https://tailwindcss.com/)
[![Vercel](https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white)](https://vercel.com/)

**AIによる音声ガイド付きストレス解消提案アプリ**
全年齢層対応のパーソナライズされた気晴らし提案サービス - 職場・学校・家庭のストレスに対応

</div>

---

## 🎯 デモサイト

<div align="center">
  <a href="https://kibarashi-app.vercel.app/" target="_blank">
    <img src="https://img.shields.io/badge/🌐%20アプリを体験する-kibarashi--app.vercel.app-blue?style=for-the-badge" alt="デモサイト">
  </a>
</div>

---

## 🌟 概要

様々な年齢層のストレスを抱える方々のために開発した、AIによる気晴らし提案アプリケーションです。Google Gemini AIを活用し、ユーザーの年齢層・状況に応じた最適なリラックス方法を提案。登録不要・完全無料で、必要な時にすぐ使えるPWAとして設計されています。

### 💡 解決する課題

- **対応年齢層**: 高校生・大学生、社会人、主婦・主夫（中学生・高齢者も企画中）
- **ターゲット状況**: 職場・学校・家庭のストレス、勉強疲れ、家事育児疲れ
- **既存アプリの問題点**: 課金ストレス、継続困難、機能過多による挫折
- **本アプリの価値**: 年齢層別最適化による即座の個別対応提案

---

## ✨ 主な機能

### 🤖 AI駆動の提案システム

- **DB-first + Ollama Cloud フォールバック**: Supabase の提案マスタ（159件）から 65ms で即時応答、未ヒット時のみ Ollama Cloud (`gemma4:31b-cloud`) で動的生成
- **年齢層別最適化**: 学生（勉強中・学校・通学中）、社会人（職場・家・外出）、主婦（家・外出）
- **Phase A完了**: 学生・社会人・主婦向けA/Bテスト済み最適化プロンプト
- **地域別カスタマイズ**: 場所変更機能により地域の天候・文化に応じた提案生成
- 認知的・行動的カテゴリーに分類された多様な提案

### 🎙️ 高度な音声ガイド機能

- **Gemini TTS** による高品質な日本語音声合成
- ブラウザTTSへの自動フォールバック機能
- タイマーと連動した音声ガイド付きリラクゼーション

### 📱 プログレッシブウェブアプリ（PWA）

- どのデバイスでもネイティブアプリのようにインストール可能
- **オフライン対応** - インターネット接続なしでも基本機能が動作
- ホーム画面への追加機能

### 🎨 モダンなUI/UX（Phase B デザインシステム刷新）

Claude Design によるプロトタイプをベースに、5フェーズのPRで全面刷新。

- **3ムード切替**: mist（ダスティブルー）/ paper（ウォームアイボリー）/ sage（ミュートグリーン）+ ダーク
- **Aurora グラデーション**: 気晴らし実行中は画面全体にゆっくりドリフトする光のレイヤー
- **呼吸する円**: 240px の breathing orb でタイマー表示、吸う/吐くリズムに同期
- **ホーム構成の選択**: CTA型（呼吸ボタン1つ）/ ステップ型 / 気分型（4つの気分タイル）
- **タイポ切替**: ゴシック（Noto Sans JP）/ 明朝（Noto Serif JP）
- **文字サイズ**: 標準 / 大きめ
- **データ属性ベースの切替**: `data-mood` / `data-dark` / `data-serif` / `data-fs` を `<html>` に付与、localStorage で永続化
- **WCAG AA準拠** のアクセシビリティ
- **場所変更機能** - ワンタップで地域を切り替え、その地域に応じた気晴らし提案を取得
- **ヘルプ機能** - アプリの使い方を分かりやすく説明するモーダル画面

### 🧪 技術的な優位性

- **95.5%のテストカバレッジ** （モック完全排除方針）
- **サーバーレスアーキテクチャ** （Vercel Functions）
- **TypeScript** による完全な型安全性
- **パフォーマンス最適化** （コード分割実装）
- **APIキーローテーション** （自動フェイルオーバー機能でレート制限対策）

### 📊 実装済み機能一覧

#### Phase 1 & 2 完了

- ✅ AI提案生成（Gemini API）
- ✅ 音声ガイド（Gemini TTS + ブラウザTTS）
- ✅ タイマー機能（開始・一時停止・リセット）
- ✅ お気に入り機能（保存・エクスポート）
- ✅ 履歴管理（統計・評価・メモ機能付き）
- ✅ カスタム気晴らし（ユーザー独自の提案登録）
- ✅ 統合データ管理（全データの一括エクスポート/インポート）
- ✅ ダークモード対応
- ✅ PWA機能（オフライン対応）

#### Phase 3 完了

- ✅ Vercel Functions移行（サーバーレス化）
- ✅ CI/CD パイプライン構築
- ✅ セキュリティスキャン自動化
- ✅ **APIキーローテーションシステム**（レート制限対策・高可用性確保）

#### Phase A 完了（年齢層別展開戦略）

- ✅ **Phase A-1**: 高校生・大学生対応（学生向け最適化）
- ✅ **Phase A-2**: 主婦・主夫対応（家事育児ストレス対応）
- ✅ 年齢層選択機能・プロファイル管理
- ✅ A/Bテストシステム（効果測定済み）
- ✅ 設定画面での年齢層切り替え機能

#### Phase B 完了（デザインシステム刷新）

- ✅ **Phase 1**: デザイントークン導入（mist/paper/sage × ダーク、spacing/radius/fs スケール）+ 設定画面に外観切替UI
- ✅ **Phase 2**: プレイヤー画面を Aurora グラデ + 呼吸する円にリデザイン
- ✅ **Phase 3**: ホーム3バリアント（CTA / ステップ / 気分）を `appearance.homeVariant` で切替
- ✅ **Phase 4**: 提案カードに kb-* トークン + aurora spot を適用
- ✅ **Phase 5**: EmptyState 共通化 + 履歴/お気に入りの空状態刷新
- ✅ **Phase 6**: アプリアイコンをパステルサンライズ配色で再生成（SVG→sharp→全サイズPNG）

#### API 外部公開（v2）

- ✅ **Supabase マイグレーション**（api_keys / api_usage_log / api_rate_limits）
- ✅ **v2 エンドポイント**: APIキー認証 + プラン別レート制限
- ✅ **v1 は完全互換維持**（既存アプリは変更不要）

#### 最新追加機能

- ✅ **47都道府県対応**（全国どこでも地域に応じた気晴らし提案）
- ✅ **場所変更機能**（位置情報に応じた気晴らし提案の最適化）
- ✅ **ヘルプモーダル**（アプリ機能の使い方説明画面）
- ✅ **DB-first 提案取得**（Supabase 提案マスタ 159件から 65ms 応答、Ollama Cloud フォールバック）
- ✅ **Ollama Cloud 移行**（Gemini API → `gemma4:31b-cloud` へ切替、コスト削減）
- ✅ **フォールバック提案の大幅拡充**（18個→65個、就活・転職者向け追加）
- ✅ **システム監視ダッシュボード**（/api/v1/dashboard でAPIキー状態・キャッシュ効率を可視化）

---

## 🛠️ 技術スタック

<div align="center">

| フロントエンド |   バックエンド   |    AI・音声    |    インフラ    |
| :------------: | :--------------: | :------------: | :------------: |
|    React 18    | Vercel Functions |  Ollama Cloud  |     Vercel     |
|   TypeScript   |     Node.js      |   Gemini TTS   | GitHub Actions |
|  Tailwind CSS  |     Supabase     | Web Speech API |  Capacitor iOS |
|      Vite      |   PostgreSQL     |                |                |
|     PWA        |                  |                |                |

</div>

### 📊 パフォーマンス指標

```
┌─────────────────────────────────────┐
│ 🚀 バンドルサイズ                    │
├─────────────────────────────────────┤
│ Vendor:  139.45 KB                  │
│ Main:     14.79 KB                  │
│ 合計:    ~154 KB (gzip圧縮後)       │
└─────────────────────────────────────┘

┌─────────────────────────────────────┐
│ ✅ テストカバレッジ                  │
├─────────────────────────────────────┤
│ 総テスト数: 707                      │
│ 成功: 701 (99.2%)                   │
│ skipped: 6                          │
│ モック使用: 最小限                  │
└─────────────────────────────────────┘

┌─────────────────────────────────────┐
│ 🌐 Lighthouse スコア                │
├─────────────────────────────────────┤
│ パフォーマンス:     95+             │
│ アクセシビリティ:   100             │
│ ベストプラクティス: 100             │
│ SEO:              100               │
│ PWA:              ✓                 │
└─────────────────────────────────────┘
```

---

## 📸 スクリーンショット

<div align="center">
  <table>
    <tr>
      <td align="center">
        <img src="docs/images/light-mode-home.png" alt="ライトモード ホーム画面" width="300">
        <br>
        <em>ライトモード - ホーム画面</em>
      </td>
      <td align="center">
        <img src="docs/images/dark-mode-suggestions.png" alt="ダークモード 提案画面" width="300">
        <br>
        <em>ダークモード - AI提案画面</em>
      </td>
      <td align="center">
        <img src="docs/images/mobile-pwa.png" alt="モバイルPWA" width="300">
        <br>
        <em>モバイルPWA体験</em>
      </td>
    </tr>
  </table>
</div>

---

## 🎯 技術的な挑戦と解決策

### 1. ゼロモックテスト哲学

**課題**: モックを一切使用せずにテストの信頼性を確保
**解決策**:

- トランザクションロールバック付きの実際のテストデータベースを実装
- 各テストスイート用の分離されたテスト環境を作成
- 実際の統合のみで95.5%のカバレッジを達成

### 2. サーバーレス環境でのGemini TTS統合

**課題**: Vercel FunctionsでGemini 2.5 Flash Preview TTSを統合
**解決策**:

- ffmpegなしでPCM音声データをWAVフォーマットに変換
- カスタムWAVヘッダー生成を実装
- ブラウザTTSへのフォールバック機構を作成

### 3. オフラインPWA機能

**課題**: 意味のあるオフライン体験の提供
**解決策**:

- インテリジェントなService Workerキャッシュ戦略を実装
- 必須の提案データを事前キャッシュ
- 同期機能付きのオフラインファーストアーキテクチャを作成

### 4. 大規模なアクセシビリティ対応

**課題**: 全機能でWCAG AA準拠を確保
**解決策**:

- CI/CDでの自動アクセシビリティテスト
- 適切なコントラスト比を持つカスタムTailwindカラーパレット
- 包括的なARIAラベリングとキーボードナビゲーション

### 5. 気晴らし提案の多様性確保

**課題**: 同じ提案が繰り返し表示される問題
**解決策**:

- Fisher-Yatesアルゴリズムによる真のランダムシャッフル実装
- フォールバックデータを20個から30個に拡充（特に30分枠を強化）
- Gemini APIプロンプトに時間ベースシードと多様性指示を追加
- 完全に一意なID生成システムの実装

### 6. Geminiプロンプトの改善によるバリエーション向上

**課題**: 提案のバリエーションが乏しく、類似した内容が繰り返される
**解決策**:

- A-san・B-sanの多様な気晴らしリストを参考にした活動細分化技術の実装
- 五感（視覚・聴覚・触覚・嗅覚・味覚）を活用した豊富な提案カテゴリの設計
- 提案履歴トラッキングシステムによる重複回避機能（最新20件保持）
- improvedPromptTemplate.tsによる時間別・状況別の詳細な活動分類
- 認知的・行動的カテゴリの細分化（想像・セルフトーク・感覚集中・運動・創作等）
- 既存のGeminiClientとの完全統合とTypeScriptエラー修正

### 7. 開発環境のポート競合と実行エラー

**課題**: ポート8080の競合とTypeScript実行エラー
**解決策**:

- バックエンドポートを8081に変更（docker-nginxとの競合回避）
- TypeScript実行をts-nodeからtsxに切り替え（ESMローダーエラーの解決）
- Gemini APIレスポンスパーサーの修正（Markdownコードブロック対応）
- フロントエンドAPIクライアントのレスポンス形式修正

### 8. 音声生成のレスポンス形式エラー

**課題**: MP3音声データをJSONとしてパースしようとしてエラー発生
**解決策**:

- TTSサービスでresponseType: 'blob'を指定してバイナリデータを直接受信
- 不要なJSONラッピングとBase64デコード処理を削除
- バックエンドの音声レスポンスとフロントエンドの期待形式を統一

### 18. フォールバック提案の大幅拡充とキャッシュシステム実装

**課題**: Gemini APIのクォータ制限によるフォールバック依存時の提案バリエーション不足
**解決策**:

- フォールバック提案を18個から65個に大幅拡充（AさんとBくんの気晴らしリストを参考）
- Fisher-Yatesアルゴリズムによる真のランダムシャッフル実装
- SuggestionCacheクラスによるGemini APIレスポンスのキャッシュ機構（TTL: 1時間）
- キャッシュヒット率の統計とダッシュボード表示
- 就活・転職活動者向けの特別カテゴリ追加

### 9. Vercelデプロイ環境での提案表示エラー

**課題**: ローカルでは動作するがVercel環境で提案が表示されない
**解決策**:

- Vercel Functionsのレスポンス形式をローカルバックエンドと統一
- 不要な`success`フィールドを削除し、データを直接返すように修正
- suggestions、tts、healthの全エンドポイントでレスポンス形式を統一

### 10. Vercel本番環境でのTailwind CSSエラー

**課題**: SuggestionDetailコンポーネントで`Cannot read properties of undefined (reading 'bg')`エラー
**解決策**:

- テンプレートリテラル内での動的クラス名生成を修正
- 条件分岐を完全なクラス名文字列に変更
- Tailwind設定にsafelistを追加して動的クラスの削除を防止

### 11. Tailwind CSSエラーの再発（SuggestionCardコンポーネント）

**課題**: SuggestionCardで`Cannot read properties of undefined (reading 'bg')`エラーが再発
**解決策**:

- categoryStylesオブジェクトのプロパティアクセスにオプショナルチェイニング（`?.`）を追加
- フォールバック値として`categoryStyles['認知的']`を設定して想定外のcategory値に対応
- ダークモードクラスを各プロパティに統合（darkBg, darkBorderプロパティを削除）

### 12. Gemini APIキャッシュ問題の修正

**課題**: 「他の提案を見る」ボタンをクリックしても同じ提案が表示される
**解決策**:

- CORSエラーの修正: バックエンドのallowedHeadersに`Cache-Control`と`Pragma`を追加
- キャッシュ無効化ヘッダーの追加: フロントエンドとバックエンドの両方で設定
- URLキャッシュバスター: タイムスタンプパラメータ`_t`を追加
- fetchオプションに`cache: 'no-store'`を設定
- 詳細なドキュメントとテストコードを作成

### 13. Phase A 年齢層別展開に伴う API スキーマ不整合

**課題**: 学生向け状況（studying, school, commuting）でAPI 400エラーが発生
**解決策**:

- フロントエンドとバックエンドの状況定義不整合を分析・特定
- バックエンドzodスキーマを拡張して学生向け状況を追加
- 3要素エラーハンドリング（何が・なぜ・どうすれば）の更新
- 既存機能（社会人向け）との完全な後方互換性を確保
- A/Bテストシステムとの統合でデータ整合性を実現

### 14. APIキーローテーションシステムの実装

**課題**: Gemini APIのレート制限によるサービス停止リスク
**解決策**:

- 複数のGoogleアカウントから取得したAPIキーの自動ローテーション機能
- APIKeyManagerクラスによる中央集権的なキー管理とシングルトンパターン
- 失敗したキーのクールダウン機能（60分間の自動無効化）
- 使用統計追跡とリアルタイム監視システム
- 管理画面での可視化とVercel環境変数設定ガイド
- 既存のGeminiClientとの完全統合（後方互換性確保）

### 15. 位置情報ベースの気晴らし提案システム

**課題**: ユーザーの地理的位置に応じた適切な気晴らし提案の生成
**解決策**:

- 場所変更機能による地域選択インターフェースの実装
- MainLayout、App、SuggestionListコンポーネント間でのlocationプロパティ伝搬
- useSuggestions hookの拡張による地域情報の API 連携
- バックエンド API の location パラメータ対応
- 地域の天候・文化・環境特性を考慮したGemini AIプロンプトの最適化
- TypeScript型安全性を保ちながらの段階的実装（オプショナルパラメータ使用）

### 16. ユーザビリティとアクセシビリティの向上

**課題**: アプリの使い方が分からないユーザーへの対応とタイトル表記の改善
**解決策**:

- アプリタイトルを「気晴らしレシピ」から時間に依存しない「気晴らし」表記に変更
- ヘルプモーダル機能の実装による機能説明画面の提供
- 直感的な場所変更ボタンのUI設計
- テキストの不適切な改行問題の修正
- アクセシビリティ配慮（キーボードナビゲーション、スクリーンリーダー対応）

### 17. 天気情報の地域別表示バグ修正

**課題**: 場所変更機能で地域を変更しても天気データが東京のまま更新されない
**解決策**:

- contextAPI.tsのキャッシュ機構に位置情報考慮の無効化ロジックを追加
- 場所が変わった場合に自動的にキャッシュをクリアする仕組みを実装
- SuggestionList.tsxで場所変更時に明示的にキャッシュをクリアする処理を追加
- Playwrightによる自動E2Eテストですべての地域で正しく天気が更新されることを確認
- バックエンドAPIは正常動作していることを確認（東京:35°C、大阪:37°C等）

---

## 🚀 学んだこと

1. **サーバーレスアーキテクチャ**: Express.jsからVercel Functionsへの移行により、コールドスタートの最適化とステートレスサービスの管理について学習
2. **AI統合とプロンプトエンジニアリング**: Gemini APIとの連携により、年齢層別最適化プロンプトとA/Bテストによる効果測定の重要性を理解
3. **テスト哲学**: ゼロモックアプローチにより、統合テストとテスト環境管理への理解が深化
4. **アクセシビリティ**: WCAG AA準拠の実装により、インクルーシブデザインの原則に関する知識が向上
5. **パフォーマンス**: 200KB未満のバンドルサイズ達成には、依存関係の慎重な分析とコード分割戦略が必要
6. **年齢層別展開戦略**: ペルソナベース設計からA/Bテスト実装まで、データ駆動型プロダクト開発の全プロセスを実践
7. **フロントエンド・バックエンド統合**: TypeScript型定義とzodスキーマの一貫性管理、API仕様の進化管理を経験
8. **高可用性システム設計**: APIキーローテーションによる障害耐性、シングルトンパターンとフェイルオーバー機構の実装を習得
9. **地域適応型AI提案**: 位置情報を活用したコンテキスト考慮型プロンプトエンジニアリングとReactコンポーネント間のデータ伝搬パターンを実践
10. **ユーザビリティ向上**: 実際のユーザーフィードバックに基づく迅速な UI/UX 改善とアクセシビリティ配慮の実装を経験

---

## 🔧 セットアップ & インストール

### 前提条件

- Node.js 20.x 以上
- npm または yarn
- **複数の**Gemini API キー（最低3つ推奨）

### クイックスタート

```bash
# リポジトリのクローン
git clone https://github.com/CaCC-Lab/kibarashi-app.git
cd kibarashi-app

# 依存関係のインストール
npm run setup

# 環境変数の設定
cp frontend/.env.example frontend/.env
# .envファイルを編集してGEMINI_API_KEY_1, GEMINI_API_KEY_2, GEMINI_API_KEY_3を追加
# 詳細はdocs/vercel-env-setup.mdを参照

# 開発サーバーの起動
vercel dev
# http://localhost:3000 にアクセス
```

### Vercel環境のデバッグ

```bash
# デバッグエンドポイントで環境変数の状態を確認
curl https://your-app.vercel.app/api/v1/debug?debug=true

# システム監視ダッシュボードにアクセス
open https://your-app.vercel.app/api/v1/dashboard

# Vercel CLIでログを確認
vercel logs --follow

# 環境変数の設定（Vercel Dashboard経由を推奨）
# 複数APIキーとローテーション設定が必要
# 詳細は docs/vercel-env-setup.md を参照
vercel env add GEMINI_API_KEY_1
vercel env add GEMINI_API_KEY_2
vercel env add GEMINI_API_KEY_3
vercel env add GEMINI_KEY_ROTATION_ENABLED true
```

### テストの実行

```bash
# 全テストを実行
npm test

# カバレッジ付きで実行
npm run test:coverage

# 特定のテストスイートを実行
npm test -- --grep "TTS"

# E2Eテスト（Playwright）を実行
npm run test:e2e
```

---

## 📁 プロジェクト構造

```
kibarashi-app/
├── frontend/          # Reactフロントエンド
├── backend/           # 従来のExpress.jsバックエンド（保持）
├── api/              # Vercel Functions（新）
│   └── v1/           # APIエンドポイント
│       ├── suggestions.ts    # 気晴らし提案API
│       ├── tts.ts           # 音声合成API
│       ├── health.ts        # ヘルスチェックAPI
│       ├── dashboard.js     # システム監視ダッシュボード
│       ├── cache-status.js  # キャッシュ状態API
│       └── _lib/            # 共通ライブラリ
│           ├── cache.js     # キャッシュシステム
│           ├── fallback.js  # フォールバック提案（65個）
│           └── apiKeyManager.js # APIキー管理
├── infrastructure/    # インフラ設定
├── docs/             # ドキュメント
│   └── vercel-env-setup.md  # Vercel環境変数設定ガイド
├── vercel.json       # Vercel設定
└── CLAUDE.md         # Claude Code用ガイド
```

詳細は [directorystructure.md](./directorystructure.md) を参照してください。

---

## 🚀 次期展開：革新的ウェルネスプラットフォーム（Phase 4）

### 世界初の職場統合型マイクロウェルネス体験

Phase 4では、現在の基本機能を基盤として**日本の職場ストレス問題を根本的に解決する革新的ウェルネスプラットフォーム**への進化を計画しています。

#### 🔮 革新的機能群（6つのコア機能）

**1. ストレス予報士AI**
- 天気予報のように個人のストレスを予測・予防
- TensorFlow.js + 時系列分析による高度な予測システム

**2. 感情錬金術ラボ**
- ネガティブ感情を創造的エネルギーに変換
- React DnD + Canvas APIによるインタラクティブ体験

**3. ソーシャル無言サポート**
- 日本の「察する文化」をデジタル化した支援ネットワーク
- WebSocketによる匿名リアルタイム支援システム

**4. 禅UI（無のインターフェース）**
- 画面を見ずに操作可能な革新的ユーザーインターフェース
- Web Audio API + ジェスチャー認識による新体験

**5. デジタル守護霊（おまもりAI）**
- 日本の「お守り」文化をAI化した精神的サポート
- パーソナライゼーションAI × 文化的コンテンツDB

**6. マイクロ予言システム**
- 5分後の自分への小さな予言による自己成就予言効果
- 行動予測AI + 心理学的フレーミング

#### 📅 実装計画
- **期間**: 7週間の集中開発スプリント
- **チーム**: 4-6名の専門開発チーム
- **投資**: ¥10,273,000-13,813,000
- **目標**: 事後対処から予防への革命的転換

#### 🎯 期待される革新的効果
- **ストレス軽減指数**: 基準値から30%改善
- **予防的介入成功率**: 70%以上
- **企業導入**: 100社以上（1年以内）
- **真のデジタルウェルビーイング**: スマホ依存を増やさない設計

詳細は以下の企画書をご参照ください：
- [革新的統合提案書_最終版.md](革新的統合提案書_最終版.md)
- [完全実装開発計画書.md](完全実装開発計画書.md)

---

## 📈 従来機能の今後の展望

- [ ] 多言語対応（英語、中国語、韓国語）
- [ ] Apple Watch / Wear OS コンパニオンアプリ
- [ ] 人気瞑想アプリとの統合
- [ ] 高度な分析ダッシュボード
- [ ] コミュニティ提供のリラクゼーション技術

---

## 🤝 コントリビューション

これは個人のポートフォリオプロジェクトですが、フィードバックや提案は歓迎します！お気軽に：

- バグや機能リクエストのIssueを開く
- 改善のためのPRを提出
- 役立つと思ったらリポジトリにスターを付ける

---

## 📝 ライセンス

このプロジェクトはMITライセンスの下で公開されています。詳細は[LICENSE](LICENSE)を参照してください。

---

## 👨‍💻 開発者について

**開発者**: CaCC-Lab
**問い合わせ先**: https://cacc-lab.net/otoiawase/
