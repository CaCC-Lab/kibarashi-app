# 気晴らしレシピアプリ アーキテクチャ設計書

## 概要

本ドキュメントは、気晴らしレシピアプリ（kibarashi-app）のシステムアーキテクチャを定義します。PWA対応のReactフロントエンドとVercel Functionsベースのサーバーレスバックエンドを中心に、外部APIとの統合を含む全体構成を説明します。
